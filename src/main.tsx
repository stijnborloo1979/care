...
const DashboardPage = lazy(() =>
  import('./features/family/FamilyPages').then((m) => ({ default: m.DashboardPage })),
)
const PeoplePage = lazy(() =>
  import('./features/family/FamilyPages').then((m) => ({ default: m.PeoplePage })),
)
const HomeMemoryPage = lazy(() =>
  import('./features/family/FamilyPages').then((m) => ({ default: m.HomeMemoryPage })),
)
const MemoriesPage = lazy(() =>
  import('./features/family/FamilyPages').then((m) => ({ default: m.MemoriesPage })),
)
const MessagesPage = lazy(() =>
  import('./features/family/FamilyPages').then((m) => ({ default: m.MessagesPage })),
)
const Settings = lazy(() => import('./features/settings/Settings'))
const ManageNotesPage = lazy(() =>
  import('./features/family/FamilyPages').then((m) => ({ default: m.NotesPage })),
)
...
function LoginPagina() {
  return (
    <main className="mx-auto max-w-[32rem] px-5 py-12">
      ...
      <p className="mt-1 text-lg text-ink-soft">Een digitaal geheugen voor elke dag.</p>
      <div className="mt-8">
        <SignIn />
      </div>
    </main>
...
function Start() {
  ...
  if (household && household.role !== 'person') return <Navigate to="/familie" replace />
  return <PersonShell />
}
  ...
  <React.StrictMode>
    ...
    >
      <AuthProvider>
        <BrowserRouter>
          <OfflineBanner />
          <Suspense fallback={<p className="p-6 text-ink-soft">Even geduld…</p>}>
            <Routes>
            ...
            >
              ...
              <Route path="/memory/ding/:itemId" element={<ItemDetail />} />
